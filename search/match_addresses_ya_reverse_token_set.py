...
INPUT_BX24 = 'df_bx24_location_geodata_v2_small_matched.json'
OUTPUT_BX24 = 'df_bx24_location_geodata_v2_small_matched_ya_reverse.json'
OUTPUT_ACCT = 'df_acct_addr_coordinates_small_matched_ya_reverse.json'
...
def best_match(target: str, candidates: List[Tuple[str, Dict[str, Any]]]) -> Tuple[Dict[str, Any], int, float]:
    ...
    for addr, row in candidates:
        ...
        if sim > best_sim:
            ...
            best_score = sim
            best_row = row
    return best_row, int(best_score) if best_score is not None else -1, best_sim
...
def main() -> None:
    ...
    bx24_candidates = [(normalize_address(r.get('ya_reverse') or ''), r) for r in bx24_rows]
    ...
    for arow in acct_rows:
        ...
        # Distance between acct(lat,lng) and bx24(lat,lng)
        try:
